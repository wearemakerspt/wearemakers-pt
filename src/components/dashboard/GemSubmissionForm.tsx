...
import { submitGem } from '@/app/dashboard/maker/actions'
...
export default function GemSubmissionForm({ spaces, existingGems }: Props) {
  ...
  const inputStyle = {
    ...
    padding: '9px 12px', fontFamily: 'var(--MONO)', fontSize: '14px',
  ...
  const pending = existingGems.filter(g => !g.is_approved)
  const approved = existingGems.filter(g => g.is_approved)
  ...
  return (
    <div style={{ padding: '14px' }}>
      ...
      {/* Submission form */}
      {open && (
        <form action={handleSubmit} style={{ padding: '16px', background: 'var(--P2)', border: '2px solid var(--INK)', marginBottom: '16px' }}>
          <div style={{ ...T, fontWeight: 700, fontSize: '10px', color: 'var(--INK)', marginBottom: '14px' }}>
            NEW HIDDEN GEM
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '12px' }}>
            ...
            <div>
              ...
              <select name="space_id" required style={{ ...inputStyle, cursor: 'pointer' }}>
                ...
                {spaces.map(s => (
                  <option key={s.id} value={s.id}>{s.name}{s.parish ? ` — ${s.parish}` : ''}</option>
                ))}
              </select>
            </div>
          ...
          {error && (
            <div style={{ ...T, fontSize: '9px', color: 'var(--RED)', fontWeight: 700, marginBottom: '10px' }}>
              ✗ {error}
            </div>
          )}
          ...
          <button
            type="submit"
            disabled={isPending}
            style={{ ...T, fontWeight: 700, fontSize: '10px', color: 'var(--P)', background: 'var(--RED)', border: '2px solid var(--RED)', padding: '10px 18px', cursor: 'pointer', opacity: isPending ? 0.6 : 1 }}
          >
          ...
          {approved.map(g => (
            <div key={g.id} style={{ padding: '10px 12px', background: 'var(--P2)', border: '1px solid rgba(24,22,20,.1)', marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '10px' }}>
              <div style={{ flex: 1 }}>
                ...
                {g.description && (
                  <div style={{ fontFamily: 'var(--MONO)', fontSize: '12px', color: 'rgba(24,22,20,.45)', marginTop: '3px' }}>{g.description}</div>
                )}
              </div>
