...
export default function BrandGallery({ photos }: Props) {
  ...
  return (
    <>
      ...
      {lightboxIndex !== null && (
        <div
          style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(18,16,14,.97)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}
          onClick={close}
        ...
        >
          {/* Top bar */}
          <div style={{ position: 'absolute', top: 0, left: 0, right: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px', zIndex: 1 }}>
            <div style={{ ...TAG, fontSize: '10px', color: 'rgba(240,236,224,.35)' }}>
              {lightboxIndex + 1} / {photos.length}
            </div>
            <button
              onClick={close}
              style={{ background: 'none', border: 'none', color: 'rgba(240,236,224,.5)', fontFamily: "'Share Tech Mono',monospace", fontSize: '11px', letterSpacing: '0.14em', textTransform: 'uppercase', cursor: 'pointer', padding: '8px' }}
            >
              CLOSE ✕
            </button>
          </div>
          ...
          {photos.length > 1 && (
            <button
              ...
              style={{ position: 'absolute', left: '12px', top: '50%', transform: 'translateY(-50%)', background: 'rgba(240,236,224,.08)', border: '2px solid rgba(240,236,224,.15)', color: '#f0ece0', fontFamily: "'Share Tech Mono',monospace", fontSize: '18px', width: '44px', height: '44px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1 }}
            >←</button>
          )}

          {/* Image */}
          <div
            style={{ maxWidth: 'min(90vw, 900px)', maxHeight: '75vh', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}
            onClick={e => e.stopPropagation()}
          >
            <img
              src={photos[lightboxIndex].photo_url}
              alt={photos[lightboxIndex].caption ?? ''}
              style={{ maxWidth: '100%', maxHeight: '70vh', objectFit: 'contain', display: 'block', border: '2px solid rgba(240,236,224,.1)' }}
            />
            {photos[lightboxIndex].caption && (
              <div style={{ ...TAG, fontSize: '10px', color: 'rgba(240,236,224,.4)', textAlign: 'center' }}>
                {photos[lightboxIndex].caption}
              </div>
            )}
          </div>
          ...
          {photos.length > 1 && (
            <button
              ...
              className="lightbox-arrow"
              style={{ position: 'absolute', right: '12px', top: '50%', transform: 'translateY(-50%)', background: 'rgba(240,236,224,.08)', border: '2px solid rgba(240,236,224,.15)', color: '#f0ece0', fontFamily: "'Share Tech Mono',monospace", fontSize: '18px', width: '44px', height: '44px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1 }}
            >→</button>
          )}
          ...
          {photos.length > 1 && (
            <div
              style={{ position: 'absolute', bottom: '24px', left: 0, right: 0, display: 'flex', justifyContent: 'center', gap: '6px' }}
              onClick={e => e.stopPropagation()}
            >
              {photos.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setLightboxIndex(i)}
                  style={{ width: i === lightboxIndex ? '20px' : '6px', height: '6px', borderRadius: '3px', background: i === lightboxIndex ? '#c8291a' : 'rgba(240,236,224,0.3)', border: 'none', cursor: 'pointer', padding: 0, transition: 'all .2s' }}
                />
              ))}
            </div>
          )}
