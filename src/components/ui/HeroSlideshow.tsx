...
interface Props {
  brands: Brand[]
  dayLabel: string   // e.g. "TUESDAY · GOOD MORNING."
  greetingLine: string // e.g. "MORN."
}
...
export default function HeroSlideshow({ brands, dayLabel, greetingLine }: Props) {
  ...
  // No brands — show pure greeting
  if (!hasBrands) {
    const parts = dayLabel.split(' · ')
    const day = parts[0] ?? ''
    return (
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', height: '100%', padding: '56px 52px 48px', borderRight: '2px solid #0C0C0C', background: '#F4F1EC' }}>
        <div style={{ fontFamily: "'Barlow Condensed',sans-serif", fontWeight: 900, fontSize: 'clamp(88px,11.5vw,168px)', lineHeight: 0.86, letterSpacing: '-0.015em', textTransform: 'uppercase' }}>
          <div style={{ color: 'rgba(12,12,12,0.13)' }}>{day}</div>
          <div style={{ color: '#1A1A1A' }}>GOOD</div>
          <div style={{ color: '#E8001C', fontStyle: 'italic' }}>{greetingLine}</div>
        </div>
        <div>
          <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: '9.5px', letterSpacing: '0.16em', color: '#6B6560', marginBottom: '6px', textTransform: 'uppercase' }}>{dayLabel}</div>
          <div style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: '10px', letterSpacing: '0.12em', color: '#1A1A1A', textTransform: 'uppercase' }}>STREET MARKETS &amp; INDEPENDENT MAKERS</div>
        </div>
      </div>
    )
  }
